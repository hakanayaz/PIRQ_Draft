...
As seen in the results LLVM IR version of the code takes too much time read and a little compilicated.

Thats why we will use `opt` to optimize the old LLVM IR to new LLVM IR.

## Middle End "opt"

In compiler insfrastructure `opt` is the middle end and what `opt` does is use the old LLVM-IR to create new LLVM-IR. `opt` is command line tool and use for visualzation, analysis, and optimization. `opt` represents the program `.dot` format. To optimize the old LLVM IR code we will use this code to create optimized version of it.

```C
opt -S -mem2reg -instnamer newton_raphson.ll -o newton_raphson_opt.ll
```

After that we will get more optimized version of the LLVM IR .ll file.
