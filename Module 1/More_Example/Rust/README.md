...
Newton Raphson method code for rust is
...
As seen in the algorithm the code has a loop and update the fucntion in side the main function.

As a next step we will focus on the IR. If you want to see the other compiler steps which are High-Level Intermadiate Representation (HIR),Typed High-Level Intermediate Representation (THIR), Middle Intermediate Representation (MIR). We will put the sub-steps results if you want to check or see.

IR representation of the code is:
